...
@click.argument('processname', type=click.STRING)
def sysmon_process_monitor(service_provider: LockdownClient, threshold, processname):
    """ monitor all most consuming processes by given cpuUsage threshold. """

    Process = namedtuple('process', 'pid name cpuUsage syscpu physFootprint')
    sys_cpu = 0.0

    with DvtSecureSocketProxyService(lockdown=service_provider) as dvt:
        with Sysmontap(dvt) as sysmon:
            for process_snapshot in sysmon.iter_sysandproc():
                #logger.info(process_snapshot)
                entries = []
                for process in process_snapshot:
                    if 'cpuUsage' in process and (process['cpuUsage'] is not None) and (process['cpuUsage'] >= threshold ) and  (process['physFootprint'] is not None) and  process['name']== processname:
                        entries.append(Process(pid=process['pid'], name=process['name'], cpuUsage=process['cpuUsage'], syscpu=sys_cpu, physFootprint=process['physFootprint']))
                        logger.debug(f'TargetProcess:{process}')
                        ...
                        logger.debug(f'sys_cpu is:{sys_cpu}')


                logger.info(entries)
#原sysmon_process_monitor函数
def sysmon_process_monitor_orign(service_provider: LockdownClient, threshold, processname):
    """ monitor all most consuming processes by given cpuUsage threshold. """
    ...
    with DvtSecureSocketProxyService(lockdown=service_provider) as dvt:
        with Sysmontap(dvt) as sysmon:
            for process_snapshot in sysmon.iter_processes():
                ...
                entries = []
                for process in process_snapshot:
                    if (process['cpuUsage'] is not None) and (process['cpuUsage'] >= threshold ) and  (process['physFootprint'] is not None) and  process['name']== processname:
